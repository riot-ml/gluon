...
let read fd ?(pos = 0) ?len buf =
  ...
  syscall ~name:"File.read" @@ fun () -> Ok (UnixLabels.read fd ~buf ~pos ~len)

let write fd ?(pos = 0) ?len buf =
  let len = Option.value len ~default:(Bytes.length buf - 1) in
  syscall ~name:"File.write" @@ fun () ->
...
let read_vectored fd iov =
  syscall ~name:"File.read_vectored" @@ fun () -> Ok (gluon_readv fd iov)

external gluon_writev : Unix.file_descr -> Iovec.t -> int = "gluon_unix_writev"

let write_vectored fd iov =
  syscall ~name:"File.write_vectored" @@ fun () -> Ok (gluon_writev fd iov)
...
let sendfile fd ~file ~off ~len =
  syscall ~name:"File.sendfile" @@ fun () -> Ok (gluon_sendfile file fd off len)
